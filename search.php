<?php
if ($_SERVER[ 'REQUEST_METHOD' ] == 'GET' && !empty($_GET['search']) && !empty($_GET['page'])) :
    ...
    $name = str_replace(' ', '%20', $_GET['search']);

    $searchMovie = curl_init();
    curl_setopt($searchMovie, CURLOPT_URL, 'https://api.themoviedb.org/3/search/movie?query='.$name.'&api_key=d040b938a74514398492c6c5acd6179f&page='.$pageNo.'');
    curl_setopt($searchMovie, CURLOPT_RETURNTRANSFER, TRUE);
    curl_setopt($searchMovie, CURLOPT_HEADER, FALSE);
    curl_setopt($searchMovie, CURLOPT_HTTPHEADER, [ 'Accept: application/json' ]);
    $searchResponse = curl_exec($searchMovie);
    curl_close($searchMovie);
    ...
    header('location: search.php?search='.$_REQUEST['search'].'&page='.$_REQUEST['page'].'');
endif;
                    ...
                    <div class='products-search__box'>
                        <form method="post" action="" autocomplete="off">
                            <div class='input-group'>
                                <input type='text' required="required" name="search" class='form-control products-search__input' placeholder='Search..' value="<?php echo $_GET['search']; ?>">
                ...
                <div class='music-section-inner'>
                    <div class='section-header d-flex align-items-center justify-content-between mb-3'>
                        ...
                        <?php if (isset($_GET['search'])) : ?>
                            <span>Search For <b>"<?php echo $_GET['search']; ?>"</b></span>
                        <?php endif; ?>
                    </div>
                    <div class='row'>
                        <?php if (isset($movies) && !empty($movies) && !empty($movies['results'])) :
                            ...
                            <div class="pagination justify-content-center">
                                <ul class='pagination'>
                                    <li class='page-item'>
                                        <a class='page-link' href='search.php?search=<?php echo $_GET[ 'search' ] ?>&page=<?php echo $page - 1; ?>'>Previous</a>
                                    </li>
                                    <?php
                                    $show = 0;
                                    for ($i = $currentPage; $i <= $total_pages; $i++) {
                                        ...
                                        if (($show < 5) || ($total_pages == $i)) {
                                            echo "<li class='page-item'><a class='page-link' href='search.php?search=" . $_GET[ 'search' ] . '&page=' . $i . "'>" . $i . '</a></li>';
                                        } else {
                                            if ($show < 6) {
                                    ...
                                    <?php } ?>
                                    <li class='page-item'>
                                        <a class='page-link' href='search.php?search=<?php echo $_GET[ 'search' ] ?>&page=<?php echo $page + 1; ?>'>Next</a>
                                    </li>
                                </ul>
                            </div>
                        <?php endif; ?>
